"""
Task 90 validation: offer accept / decline / counter work end-to-end.

Verifies:
- Only the listing seller can call offer_seller_respond (403 for others)
- accept  → offer.status == 'accepted', buyer SMS fires when opted in
...
- offer_buyer_respond: buyer decline_counter → status 'declined'
...
with app.app_context():
    ...
    with patch('routes._check_listing_csrf', return_value=None):
        ...
        # ── 2: accept → status 'accepted', buyer SMS sent ─────────────────
        ...
        check('accept: send_sms called for opted-in buyer',
              mock_sms.call_count >= 1, f'call_count={mock_sms.call_count}')
        # Verify the SMS went to the buyer's phone
        if mock_sms.call_count >= 1:
            sms_dest = mock_sms.call_args[0][0]
            check('accept: SMS destination is buyer phone',
                  sms_dest == '+16125550100', f'dest={sms_dest!r}')

        # ── 3: accept → SMS NOT sent when buyer has no SMS opt-in ─────────
        ...
        with patch('sms_service.send_sms') as mock_sms_none:
            r = client.post(
        ...
        check('accept no-sms: send_sms not called',
              mock_sms_none.call_count == 0, f'call_count={mock_sms_none.call_count}')
