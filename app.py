...
with app.app_context():
    ...
    try:
        ...
        _insp = _inspect(db.engine)
        _cols = [c['name'] for c in _insp.get_columns('listings')]
        if 'draft_last_seen_at' not in _cols:
            db.session.execute(_text(
                "ALTER TABLE listings ADD COLUMN draft_last_seen_at TIMESTAMP"
            ))
            db.session.commit()
        logging.info("Column migration: listings.draft_last_seen_at ensured")
    except Exception as _e:
        db.session.rollback()
        logging.info("Column migration (listings.draft_last_seen_at) skipped: %s", _e)
